...
Use the ▷ Run button to run the SQL script and view the results, which show the total number of trips and total revenue by month.
...
sql
 ...
 JOIN dbo.[Date] AS D
     ON T.[DateID]=D.[DateID]
 GROUP BY D.DayName;
Modify the query to add WHERE D.Month = 1. This will filter the data to include only records from the month of January. The final query should look like this:
...
sql
 ...
 WHERE D.Month = 1
 GROUP BY D.DayName
Select the text of the SELECT statement in your query. Then next to the ▷ Run button, select Save as view.
