...
In the workspace you just created, create a new Lakehouse named Sales by selecting the + New item button. Make sure the “Lakehouse schemas” option is disabled.
...
After running the last cell, select the Run tab above the ribbon and then select Stop session to stop the compute resource being used by the notebook.
...
DeltaTable.createIfNotExists(spark) \
    ...
    .execute()
In a new code block, add and run the following code to drop duplicate customers, select specific columns, and split the “CustomerName” column to create “First” and “Last” name columns:
