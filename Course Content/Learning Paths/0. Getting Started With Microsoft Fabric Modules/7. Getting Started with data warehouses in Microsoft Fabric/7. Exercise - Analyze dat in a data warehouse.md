...
GO
Use the ▷ Run button to run the SQL script, which creates a new table named DimProduct in the dbo schema of the data warehouse.
...
Navigate back to the browser tab with your warehouse and create a new SQL Query, and run the following code:
...
In the New semantic model window, name the semantic model and select all four tables. Select Confirm.
A new browser tab will automatically open with your new semantic model. In the model pane, rearrange the tables in your data warehouse so that the FactSalesOrder table is in the middle, like this:
