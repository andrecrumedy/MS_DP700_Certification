...
In the home page for your lakehouse, select Get data > New Dataflow Gen2. After a few seconds, the Power Query editor for your new dataflow opens as shown here.
New dataflow.
...
Set the New column name to MonthNo , set the Data type to Whole Number and then add the following formula: Date.Month([OrderDate]) - as shown here:
...
On the toolbar ribbon, select the Home tab. Then in the Query section, choose Add data destination.

Screenshot of the Ribbon, highlighting the Add Data destination option.

Select Lakehouse.

In the Connect to default data destination dialog box, edit the connection and sign in using your Power BI organizational account to set the identity that the dataflow uses to access the lakehouse.
...
Select Next and on the Choose destination settings page, disable the Use automatic settings option, select Append and then Save settings.
...
From your Fabric-enabled workspace, select + New item > Data pipeline, then when prompted, create a new pipeline named Load data.
